...
derive newtype instance encodeOMap ::
  ( EncodeJson a
  , EncodeJson b
  ) =>
  EncodeJson (OMap a b)

derive newtype instance decodeSession ::
  ( Ord a
  , DecodeJson a
  , DecodeJson b
  ) =>
...
fromFoldableWithIndex
  :: forall f k v. Ord k => FoldableWithIndex k f => f v -> OMap k v
...
filterKeys f om = filterWithKey (\k _ -> f k) om

filterWithKey
  :: forall k v. Ord k => (k -> v -> Boolean) -> OMap k v -> OMap k v
...
unionWith
  :: forall k v. Ord k => (v -> v -> v) -> OMap k v -> OMap k v -> OMap k v
unionWith f om1 om2 = foldMap
  (\k -> maybe mempty (singleton k) (Map.lookup k fomm))
  allKeys
  where
